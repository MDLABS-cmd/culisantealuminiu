...
export function ConfigurationSummary({
    ...
    const { state, hasCompletedRequiredSelections, markSelectionCompleted } =
        useConfigurator();

    const selectedCustomOptionIds = state.selectedCustomOptionIds;
    const selectedCustomOptions = (
        Array.isArray(activeCustomOptions) ? activeCustomOptions : []
    ).filter((option): option is CustomOption =>
        selectedCustomOptionIds.includes(option.id),
    );
    const customOfferLabel =
        selectedCustomOptions.length > 0
            ? selectedCustomOptions.map((option) => option.name).join(', ')
            : null;
    ...
    return (
        <div className="space-y-4 lg:sticky lg:top-6">
            <aside className="rounded-[20px] bg-white p-6 md:p-6">
                ...
                <Button
                    ...
                    className="poppins-regular mt-8 w-full px-10 py-2 text-[14px] leading-normal"
                ...
                <aside className="rounded-[20px] bg-white p-6 md:p-6">
                    <div className="mt-2">
                        ...
                        <p className="poppins-regular mt-6 text-[14px] leading-normal text-[#6b7280]">
                            Cererea ta a fost inregistrata pentru :{' '}
                            {customOfferLabel}
                        </p>
                    </div>
                </aside>
